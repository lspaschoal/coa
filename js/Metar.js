...
class Metar {
    ...
    constructor(mensagem) {
        ...
        mensagem.split(' ').forEach(item => {
            ...
            if (item.length === 4 && /[A-Z]{4}/.test(item) && this._icao === undefined) {
                debug(item,'ICAO');
                this._icao = item;
                return;
            }
            ...
            if (/VV/.test(item)) {
                ...
                (item === "VV///") ? this._verticalVisibility = '///': this._verticalVisibility = Number(item.substring(2)) * 100;
                return;
            }
            // Recent weather
            if (/RE/.test(item)) {
    ...
    isVMC = () => {
        return this.isCavok() || this.getVisibility() >= this._VMC.visibility && (this.getCeiling() === 'UNL' || this.getCeiling() >= this._VMC.ceiling);
    };
    isCavok = () => {
        return this._cavok;
    }
